    ...
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-500'>
        ...
        <div className='flex items-center gap-4'>
        {
            ...
            <div className='flex items-center gap-2 cursor-pointer group relative'>
                <button ><img className='w-8 rounded-full ' src={assets.profile_pic} alt='Profile-Picture'/></button>
                <img  className='h-2' src={assets.dropdown_icon} alt='Dropdown-Picture'/>
            ...
            :<button onClick={()=>navigate('/login')} className='bg-primary rounded-full text-white py-3 px-8 font-light hidden md:block'>Create Account</button>
        }
        <img onClick={()=>setShowMenu(true)} className='w-6 md:hidden' src={assets.menu_icon} alt='menu'/>
        {/*________Mobile Menu_______*/}
        <div className={`${ showMenu ? 'fixed w-full' : 'w-0 h-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white `}>
            <div className='flex items-center justify-between px-5 py-6'>
                <img className='w-36 ' src={assets.logo} alt='mm'/>
            ...
            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                ...
                <NavLink onClick={()=>setShowMenu(false)} to='/about'><p className='px-4 py-2'>ABOUT US</p></NavLink>
