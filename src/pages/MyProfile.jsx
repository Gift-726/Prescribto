...
const MyProfile = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    email: "ayanogift@gmail.com",
    phone: "+234 810 313 5256",
    address: {
      line1: "57, Ogomudia road",
      line2: "Obanla FUTA Akure"
    },
    gender: "Male",
    dob: "2002-05-03"
  ...
  return (
    <div className="max-w-lg flex flex-col gap-2 text-sm">
      ...
      {
        isEdit ? <input className="bg-gray-50 font-medium text-3xl max-w-60 mt-4" type="text" value={userData.name} onChange={e => setUserData(prev => ({ ...prev, name: e.target.value }))} /> : <p className="font-medium text-3xl text-neutral-800 mt-4">{userData.name}</p>
      }
      <hr className="bg-zinc-100 border-none h-[1px]"/>
      <div>
        <p className="text-neutral-500 underline mt-3">CONTACT INFORMATION</p>
        <div className="grid grid-cols-[3fr_3fr] gap-y-2.5 mt-3">
          <p className="font-medium">Email id:</p>
          <p className="text-blue-500">{userData.email}</p>
          <p className="font-medium">Phone:</p>
          {
            isEdit ? <input className="text-gray-500 max-w-52" type="text" value={userData.phone} onChange={e => setUserData(prev => ({ ...prev, phone: e.target.value }))} />
             : 
      ...
      <div>
        ...
        <div className="grid grid-cols-[3fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
